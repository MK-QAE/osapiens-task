...
export class BasePage {
    ...
    private startHealthMonitors() {
        ...
        this.page.on('requestfailed', request => {
            ...
            if (!request.url().includes('google-analytics') && !request.url().includes('tracker')) {
                console.log(`🚩 [BasePage] Network Failure: ${request.url()} - ${request.failure()?.errorText}`);
     ...
     */
    async navigateTo(url: string, waitCondition: 'domcontentloaded' | 'networkidle' = 'domcontentloaded') {
